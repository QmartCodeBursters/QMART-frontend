...
const QrCode = () => {
  ...
  useEffect(() => {
    const generateQRCode = async () => {
      ...
      // Construct URL for /makepayment route
      const paymentUrl = `/makepayment?amount=${amount}&businessName=${finalDetails.businessName}&accountNumber=${finalDetails.accountNumber}&walletBalance=${userDetails?.walletBalance}`;

      try {
        // Generate QR code URL with the payment URL embedded
